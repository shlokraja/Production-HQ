 ...
 *
 * DocumentStore data layer provider using mongodb.
 */
...
MongoProvider.prototype.init = function () {
    ...
    Object.keys(this._model.entitySets).map(function (key) {
        ...
        var col = new MongoCollection(key, entitySet, self._model.entityTypes[entitySet.entityType.replace("jsreport.", "")], self._options, self.db);
...
MongoCollection.prototype.update = function (query, update, options) {
    ...
    return self.db().then(function (db) {
        return self.beforeUpdateListeners.fire(query, update).then(function () {
            return q.ninvoke(db.collection(self.name), "updateMany", query, update, options).then(function(res) {
                if (!res.result.ok)
