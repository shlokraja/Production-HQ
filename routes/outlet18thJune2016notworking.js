...
// For local
// var conString = "postgres://atchayam:foodbox123@192.168.0.87:5432/foodboxdev_old";
...
router.post('/place_order', function (req, res, next)
{
    ...
    var unique_random_id =  req.body.unique_random_id;
    var barcode_dict = {};
...
{
    general.genericError("update_recovery_details: " + JSON.stringify(req.body));
    var bill_no = req.body.bill_no;
  ...
  WHERE not exists (select * from upsert);'.format(
    ...
    [outlet_id], function (err, result)
    {
        ...
        {
            console.error(err);
            general.genericError("update_recovery_details: Error" + err);
            return res.status(500).send(err);
        }
        general.genericError("update_recovery_details: success");
        res.send('success');
...
router.get('/getcashcard_sales_daymonth/:outlet_id', function (req, res, next)
{
    ...
    pg.connect(conString, function (err, client, done)
    {
        ...
        client.query('select * from ( select $1 as outlet_id, \
...
sum(case when (to_char(time,\'DD\')= to_char(now(),\'DD\') and method=\'cash\' ) then amount_collected end )as day_cash_amount, \
sum(case when (to_char(time,\'DD\')= to_char(now(),\'DD\') and method=\'card\' ) then amount_collected end ) as day_card_amount \
...
join \
(select $1 as outlet_id, \
sum(case when f.location=\'dispenser\'  then quantity else 0 end)  as dispenser_month_count, \
...
router.get('/getcashcard_sales_daymonth/:outlet_id', function (req, res, next)
{
    ...
    var outlet_id = req.params.outlet_id;
    general.genericError("getcashcard_sales_daymonth: " + outlet_id);
...
COALESCE(sum( case when method=\'cash\' then amount_collected else 0 end),0) as month_cash_amount, \
COALESCE(sum( case when method=\'card\' then amount_collected else 0 end),0) as month_card_amount, \
COALESCE(sum(case when (to_char(time,\'DD\')= to_char(now(),\'DD\') and method=\'cash\' ) then amount_collected else 0 end ),0)as day_cash_amount, \
COALESCE(sum(case when (to_char(time,\'DD\')= to_char(now(),\'DD\') and method=\'card\' ) then amount_collected else 0 end ),0) as day_card_amount \
from sales_order s, sales_order_payments p, outlet o where \
s.id=p.sales_order_id and s.outlet_id=o.id and \
to_char(time,\'MMYYYY\') = to_char(now(),\'MMYYYY\') \
and outlet_id=$1) as cc \
join \
(select $1::integer as outlet_id, \
COALESCE(sum(case when f.location=\'dispenser\'  then quantity else 0 end),0)  as dispenser_month_count,\
COALESCE(sum(case when f.location=\'outside\'  then quantity else 0 end),0)  as outside_month_count,\
COALESCE(sum(case when f.location=\'dispenser\'  then f.mrp*si.quantity else 0 end),0)  as dispenser_month_amount, \
COALESCE(sum(case when f.location=\'outside\'  then f.mrp*si.quantity else 0 end),0)  as outside_month_amount, \
COALESCE(sum(case when f.location=\'dispenser\' and to_char(time,\'DD\')=to_char(now(),\'DD\') then quantity else 0 end),0)  as dispenser_day_count,\
COALESCE(sum(case when f.location=\'outside\' and to_char(time,\'DD\')=to_char(now(),\'DD\') then quantity else 0 end),0)  as outside_day_count, \
COALESCE(sum(case when f.location=\'dispenser\' and to_char(time,\'DD\')=to_char(now(),\'DD\') then f.mrp*si.quantity else 0 end),0)  as dispenser_day_amount,\
COALESCE(sum(case when f.location=\'outside\' and to_char(time,\'DD\')=to_char(now(),\'DD\') then f.mrp*si.quantity else 0 end),0)  as outside_day_amount \
from food_item f, \
      sales_order_items si, sales_order s where s.id=si.sales_order_id and \
      si.food_item_id=f.id and s.outlet_id=f.outlet_id and s.outlet_id=$1 \
