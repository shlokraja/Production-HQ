...
var get_bill_bundle = function(bill_data, callback) {
  ...
  _.each(_.keys(grouped), function(key){
    ...
    // GST Changes
    //console.log("moment(sample.time,'YYYY-MM-dd')");
    //console.log(moment(sample.time,'YYYY-MM-dd'));
    if ( moment(sample.time).format('YYYY-MM-DD')>=  gst_date){
   //   console.log("INSIDE GST moment(sample.time,'YYYY-MM-dd')");
      // bill["sgst_perc"] = 9;//sample.vat_perc;
      // bill["cgst_perc"] = 9;//sample.vat_perc;      
      // bill["gstin_no"] = "123456789ABCXYZ";
      // bill["entity"]=sample.fv_name;    
