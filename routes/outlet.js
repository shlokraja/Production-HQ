      ...
      RETURNING id',
                    ...
                    async.map(_.keys(food_details),
                        function (item_id, map_callback) {
                          ...
                          VALUES ($1, $2, $3, $4, get_dispense_status($3)::dispense_status,$5,$6)', [sales_order_id,
                                ...
                                redisClient.lrange(restaurant_id + "_" + outlet_id, 0, -1, function (err, data) {
                                    if (err) {
                                        console.log(err);
                                    }
                                    ...
                                    if (data.length > 0) {
                                        for (var i = 0; i < data.length; i++) {
                                            ...
                                            if (data[i].billno == bill_no) {
                                                serial_no = data[i].serialno;
                                                redisClient.lrem(restaurant_id + "_" + outlet_id, 1, remData);
                                            }
                                        }
                                        config.query('INSERT INTO bill_items \
              VALUES ($1, $2, $3, $4, get_dispense_status($3)::dispense_status,$5,$6,$7)', [sales_order_id,
                                                bill_no, item_id, food_details[item_id], mobile_num, is_mobile_order, serial_no], map_callback);
                                    }
                                    else {
                                        config.query('INSERT INTO bill_items \
              VALUES ($1, $2, $3, $4, get_dispense_status($3)::dispense_status,$5,$6,$7)', [sales_order_id,
        ...
        async.map(_.keys(dispense_status_data),
            function (bill_no, map_callback) {
                ...
                console.log("bill no update:" + bill_no +" Oulet_id:"+outlet_id);
//if  (dispense_status_data[bill_no]=='timeout' ) return; //********todo
          ...
          and sales_order_id=(select max(id) \
            ...
            and s.outlet_id=$3 \
            and b.bill_no=$2 and s.time > now() -interval '6' hour )",
            ...
            where bill_no=$1 and sales_order_id=(select max(id) from sales_order s, bill_items b where s.id=b.sales_order_id and s.outlet_id=$2 and b.bill_no=$1 and s.time > now() -interval '6' hour ) and is_mobile_order=TRUE",
...
router.get('/dispenser_queue/:outlet_id', function (req, res, next) {
    ...
    and s.time > (now() - interval \'8 hours\') \
