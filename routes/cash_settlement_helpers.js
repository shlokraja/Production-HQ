...
var fetchPurchaseOrders = function(outlet_id, date, async_callback) {
  pg.connect(conString, function(err, client, done) {
    ...
    client.query(
      ...
      COALESCE(fi.cgst_percent,0) as cgst_perc, \
      COALESCE(fi.sgst_percent,0) as sgst_perc, \
...
var mapToBucket = function(purchase_item) {
  if(_.contains(["sold"], purchase_item.status)) {
  ...
  } else if(_.contains(["undelivered", "damaged in transit", "damaged"], purchase_item.status)) {
    ...
    "loading_issue", "quantity", "quality", "packing","restaurantfault"], purchase_item.status)) {
    purchase_item.bucket = "restaurant";
  } else if(_.contains(["scanner fault (Foodbox fault)", "damaged while dispensing","unscanned"], purchase_item.status)) {
...
var per_item_accounting = function(purchase_item, tds_perc, abatement_perc, fbx_st_perc, gst_percent) {
  var cash_settlement = {};
  ...
  var qty = purchase_item.qty;
    // new GST percents    
  var fi_gst_perc=   purchase_item.cgst_perc+purchase_item.sgst_perc;

  // If Food Item GST is 0 then it will take GST from Abatement table for city.
  var gst_perc = fi_gst_perc==0? gst_percent:fi_gst_perc;
  var abate_gst_perc = gst_percent;
  ...
  cash_settlement["restaurant_tax_remit"] = cash_settlement["vat"] + cash_settlement["st_with_abatement"];
    ...
    // Include the GST percents
  cash_settlement["foodbox_gst"] = cash_settlement["foodbox_fee"] * abate_gst_perc / 100;
  cash_settlement["foodbox_txn_gst"] = cash_settlement["foodbox_fee"] + cash_settlement["foodbox_gst"];
  cash_settlement["restaurant_fee_gst"] = cash_settlement["sale"] - cash_settlement["foodbox_txn_gst"];
  cash_settlement["restaurant_remit_bef_adj_gst"] = cash_settlement["restaurant_fee_gst"] + cash_settlement["foodbox_tds"];
  cash_settlement["restaurant_liability_gst"] = cash_settlement["restaurant_liability"] * gst_perc / 100;
  cash_settlement["restaurant_liability_total_gst"] = cash_settlement["restaurant_liability"]
   + cash_settlement["restaurant_liability_gst"] - cash_settlement["restaurant_liability_tds"];
  cash_settlement["restaurant_liability_net_gst"] = cash_settlement["restaurant_liability_total_gst"]
  - cash_settlement["foodbox_liability"];
  cash_settlement["restaurant_remit_aft_adj_gst"] =
  cash_settlement["restaurant_remit_bef_adj_gst"] - cash_settlement["restaurant_liability_net_gst"];

  cash_settlement["gst"] = cash_settlement["sale"] * gst_perc / 100;
  cash_settlement["restaurant_tax_remit_gst"] = cash_settlement["gst"];
  // Add cash settlement to each purchase item.
  purchase_item.cash_settlement = cash_settlement;
};

var consolidate_po_item_accounting = function(purchase_orders, outlet, tds_perc, abatement_perc, fbx_st_perc, gst_perc) {
  ...
  _.each(purchase_orders, function(purchase_item){
    ...
    per_item_accounting(purchase_item, tds_perc, abatement_perc, fbx_st_perc, gst_perc);
  });
};

var consolidate_so_item_accounting = function(sales_orders, outlet, tds_perc, fbx_st_perc, fbxFV, gst_perc) {
  ...
  _.each(_.keys(grouped), function(key){
    ...
    outside_order["vat_perc"] = sample.vat_perc;
      // GST integration
    outside_order["gst_perc"] = sample.cgst_perc + sample.sgst_perc;
  ...
  _.each(non_dispenser_sales, function(outside_order) {
    ...
    var eff_gst_perc = (fbxFV.id == outside_order.restaurant_id)?0:gst_perc;
    ...
    cash_settlement["foodbox_gst"] = cash_settlement["foodbox_fee"]*eff_gst_perc/100;
    cash_settlement["foodbox_txn"] = cash_settlement["foodbox_fee"] + cash_settlement["foodbox_st"];
    cash_settlement["foodbox_txn_gst"] = cash_settlement["foodbox_fee"] + cash_settlement["foodbox_gst"];
    cash_settlement["restaurant_fee"] = cash_settlement["sale"] - cash_settlement["foodbox_txn"];
    cash_settlement["restaurant_fee_gst"] = cash_settlement["sale"] - cash_settlement["foodbox_txn_gst"];
    ...
    cash_settlement["restaurant_liability_gst"] = cash_settlement["restaurant_liability"]*eff_gst_perc/100;
...
var process_and_store_cash_settlement = function(date,
  ...
  tds_perc, abatement_perc, fbx_st_perc, fbxFV, outlet_sessions, gst_perc, async_callback) {

  // Add session to po item
  compute_session_for_po(purchase_orders, outlet_sessions);

  this.consolidate_po_item_accounting(purchase_orders, outlet, tds_perc, abatement_perc, fbx_st_perc, gst_perc);
  var non_dispenser_sales = this.consolidate_so_item_accounting(outside_sales, outlet, tds_perc, fbx_st_perc, fbxFV,gst_perc);
